...
begin
  Infos.Clear;
  // Get the size of the FileVersionInfo struct
  len := GetFileVersionInfoSize( PChar(FileName), hdl );
  if len = 0 then
    Exit;

  // Allocate memory for the FileVersionInfo struct
  GetMem( pInfos, len );
  try
    // Get the FileVersionInfo struct
    if GetFileVersionInfo( PChar(FileName),
                           hdl,
                           len,
                           pInfos ) then
      begin
        buff_len := sizeof(pbuff);
        // Define the Language and Charset string
        if VerQueryValue( pInfos, PChar('\VarFileInfo\Translation'), pbuff, buff_len ) then
          begin
            if buff_len > 0 then
              begin
                packet := TPacket(pbuff^);
                with packet do
                  HexNumber := b2 + b3 * $100 + b0 * $10000 + b1 * $1000000;
                sLanguage := Format('%x', [HexNumber]);
                // pad the string with 0 on the left if needed
                while length(sLanguage) < 8 do
                  Insert( '0', sLanguage, 1 );

                // Get the fields of the FileVersionInfo struct
                // and put it in the argument list Infos as :
                // field = value

                for i := Low(sVersionInfo) to High(sVersionInfo) do
                  begin
                    sField := '\StringFileInfo\'+sLanguage+'\'+sVersionInfo[i];
                    buff_len := 255;
                    SetLength( buff, buff_len );
                    rc := VerQueryValue( pInfos, PChar(sField), Pointer(buff), buff_len );
                    if rc then
                      begin
                        SetLength( buff, buff_len );
                        Infos.Values[sVersionInfo[i]] := buff;
                      end; // if field found
                  end; // for each field to recover
              end; // if buff_len > 0
...
begin
  if GetDiskFreeSpace( TheDrive, sectors, bytesPerSector, freeClusters, clusters ) then
    begin
      TotalBytes := sectors * bytesPerSector * clusters;
      TotalFree  := sectors * bytesPerSector * freeClusters;
    end
  else
    begin
      TotalBytes := -1;
